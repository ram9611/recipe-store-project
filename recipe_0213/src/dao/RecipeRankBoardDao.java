...
/*			String sqlForRecipe =
					"SELECT t2.* \r\n" + 
							...
							"FROM RECIPE r join RECIPE_HIT_DATE rhd on (r.recipe_id = rhd.recipe_id) \r\n" + 
							"join MEMBER m on (r.u_id = m.u_id) \r\n" + 
							"where (r.magazine_id = 0 AND r.DELETEDPOST != 1 AND r.DELETEDACCOUNT != 1 AND r.complete = 1 AND ("+?+")) group by r.recipe_id, r.THUMBNAIL, r.title, r.star, m.nickname, m.PROFILE_IMAGE \r\n" + 
							"ORDER BY count(rhd.hit_date) DESC, r.star DESC) t)t2 \r\n" + 
							"WHERE (t2.rnum >= 1 AND t2.rnum<= 100)";*/
...
/*			String sqlForSearchWord = 
					"SELECT t2.* FROM \r\n" + 
							...
							"ORDER BY count(searchword) DESC) t)t2 \r\n" + 
							"WHERE (t2.rnum >= 1 AND t2.rnum <= 100)";*/
			PreparedStatement pstmt = ConnectionRecipe.getConnection().prepareStatement(sqlForSearchWord);
			ResultSet rs = pstmt.executeQuery();
			while(rs.next()) { 
